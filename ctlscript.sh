...
INSTALLDIR=/home/css/edx-edutech
...
#xqueue_code_start
XQUEUE_SCRIPT=/home/css/edx-edutech/apps/xqueue/scripts/ctl-xqueue-consumer.sh
#xqueue_code_end
#forum_code_start
FORUM_SCRIPT=/home/css/edx-edutech/apps/forum/scripts/ctl.sh
#forum_code_end
#edx_code_start
EDX_SCRIPT=/home/css/edx-edutech/apps/edx/scripts/ctl-edx-celery-workers.sh
#edx_code_end
...
help() {
  ...
  if test -x $POSTGRESQL_SCRIPT; then
	...
	if test -x $LIBREOFFICE_SCRIPT; then
        ...
        #xqueue_code_start
        if test -x $XQUEUE_SCRIPT; then
            echo "       $0 (start|stop|restart|status) xqueue"
        fi
        #xqueue_code_end
        #forum_code_start
        if test -x $FORUM_SCRIPT; then
            echo "       $0 (start|stop|restart|status) forum"
        fi
        #forum_code_end
        #edx_code_start
        if test -x $EDX_SCRIPT; then
            echo "       $0 (start|stop|restart|status) edx"
        fi
        #edx_code_end
...
#xqueue_code_start
    if [ "x$XQUEUE_ERROR" != "x" ] && [ $XQUEUE_ERROR -gt 0 ]; then
        ERROR=$XQUEUE_ERROR
    fi
    #xqueue_code_end
    #forum_code_start
    if [ "x$FORUM_ERROR" != "x" ] && [ $FORUM_ERROR -gt 0 ]; then
        ERROR=$FORUM_ERROR
    fi
    #forum_code_end
    #edx_code_start
    if [ "x$EDX_ERROR" != "x" ] && [ $EDX_ERROR -gt 0 ]; then
        ERROR=$EDX_ERROR
    fi
    #edx_code_end
