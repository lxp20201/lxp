#!/bin/bash
XQUEUE_CONSUMER_START="/home/css/edx-edutech/apps/xqueue/bin/python.xqueue manage.py run_consumer >/dev/null 2>&1 &"
XQUEUE_CONSUMER_STATUS=""
ERROR=0
is_xqueue_consumer_running() {
    if [[ -n $(ps axf | grep "/home/css/edx-edutech/apps/xqueue/venvs/xqueue/bin/.python2.7.bin" | grep -v grep) ]]; then
        RUNNING=1
...
stop_xqueue_consumer() {
    ...
    ps axf | grep "/home/css/edx-edutech/apps/xqueue/venvs/xqueue/bin/.python2.7.bin" | grep -v grep |  awk '{print "kill -9 " $1}' | sh
    is_xqueue_consumer_running
    RUNNING=$?
...
start_xqueue_consumer() {
    ...
    if [ `id|sed -e s/uid=//g -e s/\(.*//g` -eq 0 ]; then
        su -s /bin/sh css -c "eval $XQUEUE_CONSUMER_START"
    else
